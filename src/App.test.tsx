...
describe('Home view', () => {
  it('should render 20 comics', async () => {
    const fetcher = new Fetcher<ApiResponse<ComicInterface[]>>()
    const json_test: ApiResponse<ComicInterface[]> = require('./test/test_comics.json')
    jest.spyOn(fetcher, 'fetch').mockImplementation(() => Promise.resolve(json_test))
    render(
      <BrowserRouter>
...
describe('Character view', () => {
  it('should render character information', async () => {
    const fetcher = new Fetcher<ApiResponse<CharacterInterface[]>>()
